...
def analyze_section(section: Section, subgrade_E: float = STANDARD_SUBGRADE_E, 
    ...
    """
    # Convert thickness from cm to meters (1 cm = 0.01 m)
    CM_TO_METER = 0.01
    
    # Extract layer data from Section
    nL = len(section)
    # Use thickness_cm (integer values in cm) and convert to meters
    Thicknesslayers_orig = np.array([layer.thickness_cm * CM_TO_METER for layer in section])
