...
def make_possible_sections(material_list:list,num_capas:int) -> list:
    """
    ->Return list[Section] \n
    ...
    indices[:] = [x for x in indices if x[0] in surface_lst]
    g=0
    tam = len(indices)
    while g<tam:
        surface=0
        ...
        for h in indices[g]:
            ...
            if h in alkaline_lst:
                #Tecnicamente se pueden tener dos capas alcalinas si estas no se tocan. Pero no se implementa xd
                alk +=1
            if h in subgrad_lst:
                trat_sub+=1  
